...
Microcontroller::Microcontroller()
    : control_mode_(0), fault_flags_(0), control_cycles_(0), uptime_seconds_(0),
      ...
      gyro_rates_{0.0f, 0.0f, 0.0f}, magnetometer_{0.0f, 0.0f, 0.0f},
      wheel_torques_{0.0f, 0.0f, 0.0f}, magnetorquer_{0.0f, 0.0f, 0.0f}, sun_angle_(0.0f)
{
...
void Microcontroller::processSensorData(const SensorData& sensor_data) {
    if (!sensor_data.valid) {
        consecutive_faults_++;
        if (consecutive_faults_ > 5) control_mode_ = 0; // Safe mode
        return;
    }
...
void Microcontroller::setFaultThreshold(float threshold) {
    if (threshold > 0.0f) fault_threshold_ = threshold;
}
...
void Microcontroller::performFaultDetection() {
    ...
    if (sensor_fault) fault_flags_ |= 0x08;
...
void Microcontroller::updateHealthMonitoring() {
    if (control_cycles_ % 10 == 0) uptime_seconds_++;
}
